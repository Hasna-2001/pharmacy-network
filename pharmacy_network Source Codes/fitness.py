...
def fitness_store(tab,notebook,home,epharmacies,cart,path,bucket,cart_func,orders,offers,signin):
    ...
    image_path1 = path('fit2.png')
    tab.medicaplogo = ctk.CTkImage(light_image=Image.open(image_path1),size=(160,82))
...
#########################################################################
    ...
    ctk.CTkLabel(type_frame1,text="PROTEIN POWDERS",text_color="black",
    ...
    ctk.CTkLabel(type_frame2,text="PROTEIN FOODS",text_color="black",
                 font=("impact",30)).grid(row=0,column=0,padx=5,pady=10)
...
#############################################################################################
    ...
    item_frames = [type_frame1,type_frame2]
    def run():
        i=0
        connection = sqlite3.connect("pharmacy.db")
        cursor = connection.cursor()

        cursor.execute("select name, price from protein_powders")
        items = cursor.fetchall()
        for item in items:
            protein_powders[item[0]]=item[1]

        cursor.execute("select name, price from protein_bars")
        items = cursor.fetchall()
        for item in items:
            protein_bars[item[0]]=item[1]

        connection.commit()
        connection.close()

        all_items=[protein_powders,protein_bars]
