...
def create_username2(tab,home,back,notebook,restart,details,pharmacient_details,run_admin_table,refresh_remove,state):
    ...
    def add():
        if user_var.get()=="" or password_var.get()=="" or conform_var.get()=="":
            ...
            place_details=(details[0],details[1],details[2],details[4],details[5])
            connection = sqlite3.connect("pharmacy.db")
            cursor = connection.cursor()
            
            cursor.execute("insert into nearby_pharmacy values(?,?,?,?,?)",place_details)
            cursor.execute("insert into images (name,image)values(?,?)",(details[0],details[3]))
            cursor.execute("insert into pharmacient_names (name)values(?)",(pharmacient_details[0],))
