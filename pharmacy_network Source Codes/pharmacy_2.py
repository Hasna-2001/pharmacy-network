...
def medicap(tab,notebook,home,epharmacies,cart,path,bucket,cart_func,orders,offers,login):
    ...
    ctk.CTkLabel(temp_frame,text="",image=tab.medicaplogo,text_color="blue",fg_color="#EAEAEA",
                 font=("impact",20),height=0).grid(row=0,column=1,padx=50,pady=5)

    ctk.CTkButton(temp_frame,text="",fg_color = "#EEEEEE",text_color="black",image=tab.usericon,hover_color="#EAEAEA",
                  command=lambda:notebook.select(login)).grid(row=0,column=2,sticky="e")
...
#########################################################################
    ...
    ctk.CTkLabel(type_frame1,text="CAPSULES",text_color="black",
    ...
    ctk.CTkLabel(type_frame3,text="DROPS",text_color="black",
                 font=("impact",30)).grid(row=0,column=0,padx=5,pady=10)
...
#############################################################################################
    ...
    def insert_to_show(items,type_of_item):
        ...
        for name,price in items.items():
            ...
            ctk.CTkButton(item1,textvariable=item_var1,width=190,compound="top",image=img,fg_color="white",
                            height=190,text_color="black",border_color="#dd0e44",hover_color="white",
                          border_width=2,font=("roboto",15,"bold")).pack(pady=5,padx=10)
    ...
    def run_medicap():
        ...
        cursor.execute("select name, price from capsules")
        items = cursor.fetchall()
        for item in items:
            capsules[item[0]]=item[1]


        cursor.execute("select name, price from drops")
        items = cursor.fetchall()
        for item in items:
            drops[item[0]]=item[1]
