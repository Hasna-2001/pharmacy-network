...
def pharm_easy(tab,signin,signup,notebook,home,
               epharmacies,cart,path,cart_func,orders,offers):
    ...
    #body frame 
    body_frame = ctk.CTkFrame(pharm_easy,fg_color = "#EAEAEA")
    ...
    #back to menue
    temp_frame=ctk.CTkFrame(body_frame,fg_color = "#EAEAEA")
    temp_frame.grid(row=0,column=0,padx=15,sticky="we",pady=10)
    temp_frame.columnconfigure(2,weight=2)
    
    ctk.CTkLabel(temp_frame,text="",image=tab.pharm_easy_logo,text_color="blue",fg_color="#EAEAEA",
                 font=("impact",20),height=0).grid(row=0,column=1,padx=50)

    
    ctk.CTkButton(temp_frame,text="",fg_color = "#EAEAEA",text_color="black",image=tab.usericon,hover_color="#EAEAEA",
                  command=lambda:notebook.select(signin)).grid(row=0,column=2,sticky="e")
        
    

    #all item frame
    item_fram = ctk.CTkScrollableFrame(body_frame,fg_color="#EAEAEA",height=540)
    item_fram.grid(row=2,column=0,sticky="wens",padx=20)
    item_fram.columnconfigure(0,weight=1)
...
#########################################################################
    ...
    type_frame1.grid(row=1,column=0,padx=5,pady=30,sticky="w")
    type_frame1.columnconfigure(0,weight=2)
    ctk.CTkLabel(type_frame1,text="TABLETS",text_color="black",
                 font=("impact",30)).grid(row=0,column=0,padx=5,pady=10)


    #specific type frame 3
    type_frame3=ctk.CTkFrame(item_fram,width=300,fg_color="#AFDDE5")
    type_frame3.grid(row=3,column=0,padx=5,pady=30,sticky="w")
    type_frame3.columnconfigure(0,weight=2)
    ctk.CTkLabel(type_frame3,text="VITAMINS",text_color="black",
                 font=("impact",30)).grid(row=0,column=0,padx=5,pady=10)
...
#############################################################################################
    ...
    def insert_to_show(items,type_of_item):
        ...
        for name,price in items.items():
            ...
            ctk.CTkButton(item1,textvariable=item_var1,width=190,compound="top",image=img,fg_color="white",hover_color="white",
                            height=190,text_color="black",border_color="#dd0e44",border_width=2,font=("roboto",15,"bold")).pack(pady=5,padx=10)
    ...
    item_frames = [type_frame1,type_frame3]
    def run():
        ...
        cursor.execute("select name, price from pharmeasy_tablets")
        items = cursor.fetchall()
        for item in items:
            pharmeasy_tablets[item[0]]=item[1]

        connection = sqlite3.connect("pharmacy.db")
        cursor = connection.cursor()

        cursor.execute("select name, price from vitamins")
        items = cursor.fetchall()
        for item in items:
            VITAMINS[item[0]]=item[1]
        
        connection.commit()
        connection.close()
   
        all_items=[pharmeasy_tablets,VITAMINS]
