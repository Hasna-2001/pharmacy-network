...
def create_username(tab,home,back,notebook,restart,details,run_table,refresh_remove_doc,state):
    ...
    def add():
        if user_var.get()=="" or password_var.get()=="" or conform_var.get()=="":
            ...
            doctor_details=(details[0],details[1],details[2],details[4])
            connection = sqlite3.connect("pharmacy.db")
            cursor = connection.cursor()
            cursor.execute("insert into doctors_details values(?,?,?,?)",doctor_details)
            cursor.execute("insert into images values(?,?)",(details[0],details[3]))
            cursor.execute("insert into doctor_names (name) values(?)",(details[0],))
