...
#mosturizers changed into scrubs 


def skincare_shop(tab,notebook,home,epharmacies,cart,path,bucket,cart_func,orders,
                  offers,signin):
    ...
    image_path1 = path('skin_care2.png')
    tab.medicaplogo = ctk.CTkImage(light_image=Image.open(image_path1),size=(130,130))
    ...
    ctk.CTkLabel(temp_frame,text="",image=tab.medicaplogo,fg_color="#EEEEEE",
                 font=("roboto",25,"bold"),height=0,text_color="#0b7992").grid(row=0,column=1,padx=50)
...
#########################################################################
    ...
    ctk.CTkLabel(type_frame3,text="SKRUBS",text_color="black",
                 font=("impact",30)).grid(row=0,column=0,padx=5,pady=10)

    type_frame4=ctk.CTkFrame(item_fram,width=300,fg_color="#AFDDE5")
    type_frame4.grid(row=4,column=0,padx=5,pady=20,sticky="w")
    type_frame4.columnconfigure(0,weight=2)
    ctk.CTkLabel(type_frame4,text="SERUMS",text_color="black",
                 font=("impact",30)).grid(row=0,column=0,padx=5,pady=10)

    type_frame5=ctk.CTkFrame(item_fram,width=300,fg_color="#AFDDE5")
    type_frame5.grid(row=5,column=0,padx=5,pady=20,sticky="w")
    type_frame5.columnconfigure(0,weight=2)
    ctk.CTkLabel(type_frame5,text="SUNSCREEN",text_color="black",
                 font=("impact",30)).grid(row=0,column=0,padx=5,pady=10)

    type_frame6=ctk.CTkFrame(item_fram,width=300,fg_color="#AFDDE5")
    type_frame6.grid(row=6,column=0,padx=5,pady=20,sticky="w")
    type_frame6.columnconfigure(0,weight=2)
    ctk.CTkLabel(type_frame6,text="LIP BALM",text_color="black",
                 font=("impact",30)).grid(row=0,column=0,padx=5,pady=10)
...
#############################################################################################
    ...
    def insert_to_show(items,type_of_item):
        ...
        for name,price in items.items():
            ...
            ctk.CTkLabel(qty_frame,text="Quantity: ",font=("roboto",15,"bold"),text_color="#60044a").pack(side="left",padx=5)
    ...
    item_frames = [type_frame3,type_frame4,type_frame5,type_frame6]
    def run():
        i=0
        connection = sqlite3.connect("pharmacy.db")
        cursor = connection.cursor()

        cursor.execute("select name, price from mosturaizers")
        items = cursor.fetchall()
        for item in items:
            mosturaizers[item[0]]=item[1]

        cursor.execute("select name, price from serums")
        items = cursor.fetchall()
        for item in items:
            serums[item[0]]=item[1]

        cursor.execute("select name, price from sunscreen")
        items = cursor.fetchall()
        for item in items:
            sunscreen[item[0]]=item[1]

        cursor.execute("select name, price from lip_balm")
        items = cursor.fetchall()
        for item in items:
            lip_balm[item[0]]=item[1]


        connection.commit()
        connection.close()

        all_items=[mosturaizers,serums,sunscreen,lip_balm]
