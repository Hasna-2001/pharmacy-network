...
def pharmacient_form(tab,login,home,notebook,path,userfrom,doctor_frame,form):

        pharmacient_fram = ctk.CTkScrollableFrame(tab,fg_color="#EAEAEA")
        pharmacient_fram.pack(fill="both",expand=True)

        pharmacient_fram.rowconfigure(0,weight=0)
        pharmacient_fram.rowconfigure(1,weight=0)
        pharmacient_fram.columnconfigure(0,weight=2)

        user_form1 = ctk.CTkFrame(pharmacient_fram,fg_color="white")
        ...
        def validate():
                if first_name.get()=="" or address_value.get()=="" or phone_value.get()=="" or last_name.get()=="" or dob.get()=="" or address_value.get()=="" or email_value.get()=="" or phone_value.get()=="" or workplace_value.get()=="" or licence_value.get()=="":
        ...
        role_value = tk.StringVar(value="PARMECIENT")
        ...
        ctk.CTkLabel(user_frame,text="JOIN AS A PHARMACIENT",text_color="black",font=("calibri",20,"bold")).grid(row=0,column=0,columnspan=3,pady = 20)

        ctk.CTkLabel(user_frame,text="Name: ",text_color="black").grid(row=1,column=0,padx=30)

        
        first_name = ctk.CTkEntry(user_frame,placeholder_text="FIRST NAME",text_color="black",font=("calibri",16),
                                  fg_color="white",border_color="#DB0A41",corner_radius=50,height=35,
                                  width=200)
        first_name.grid(row=1,column=1,pady=10,sticky="w",padx=10)

        
        last_name = ctk.CTkEntry(user_frame,placeholder_text="LAST NAME",text_color="black",font=("calibri",16),
        ...
        phone = ctk.CTkEntry(user_frame,textvariable=phone_value,text_color = "black",font=("calibri",16),width=300,fg_color="white",border_color="#DB0A41",corner_radius=50,height=35,)
        ...
        ctk.CTkLabel(user_frame,text="Pharmacy\nLicence Number: ",text_color="black").grid(row=8,column=0)
        licence_value = tk.StringVar()
        licence = ctk.CTkEntry(user_frame,textvariable=licence_value,text_color = "black",font=("calibri",16),width=300,fg_color="white",border_color="#DB0A41",corner_radius=50,height=35,)
        licence.grid(row=8,column=1,columnspan=2,pady=10,sticky="w",padx=10)
        
        btn_farme= ctk.CTkFrame(user_frame,fg_color="white")
        btn_farme.grid(row=9,column=1,columnspan=3,pady=10)
