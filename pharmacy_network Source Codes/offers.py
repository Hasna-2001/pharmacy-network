...
def show_offer(tab,notebook,home,epharmacies,cart,path,cart_func,my_orders,bucket):

    #main frame in pharm easy
    medicap = ctk.CTkFrame(tab,fg_color = "#EAEAEA")
    ...
    nav=ctk.CTkFrame(medicap,fg_color="#3B1E54",corner_radius=0)
    nav.grid(row=0,column=0,sticky="we")
    nav.columnconfigure(0,weight=2)

    ctk.CTkLabel(nav,text="OFFERS",
                 font=("impact",35),text_color="white").grid(row=0,column=0,pady=5)

    #body frame 
    body_frame = ctk.CTkFrame(medicap,fg_color = "#EAEAEA")
    ...
    image_path1 = path('medicap.png')
    tab.medicaplogo = ctk.CTkImage(light_image=Image.open(image_path1),size=(180,80))
    ...
    #all item frame
    item_fram = ctk.CTkScrollableFrame(body_frame,fg_color="#EAEAEA",height=550)
    item_fram.grid(row=0,column=0,sticky="wens",padx=20)
...
#########################################################################
    ...
    type_frame1=ctk.CTkFrame(item_fram,width=300,fg_color="#EAEAEA")
...
#############################################################################################
    ...
    def insert_to_show(item,type_of_item):
        ...
        for name,price in item.items():
            ...
            cursor.execute("SELECT offer FROM offers WHERE name = ?", (name,))
            ...
            item_btn = ctk.CTkButton(item1,textvariable=item_var1,width=190,compound="top",image=img,fg_color="white",hover_color="white",
                            height=190,text_color="black",border_color="#dd0e44",border_width=2,font=("roboto",15,"bold"))
            item_btn.pack(pady=5,padx=10)
            ctk.CTkLabel(item_btn,text=f"-{offer[0]}%",fg_color="green",corner_radius=5,font = ("roboto",15,"bold")).place(relx=0.05,rely=0.05)
    ...
    def run_offer():
            ...
            connection = sqlite3.connect("pharmacy.db")
            cursor = connection.cursor()
            cursor.execute("select name,price from offers")
            items = cursor.fetchall()

            for item in items:
                offer_items[item[0]]=item[1]
            
            insert_to_show(offer_items,type_frame1)
    ...
    ctk.CTkButton(temp_footer,text="",width=0,image=tab.offers_logo,fg_color = "#3B1E54").grid(row=0,column=4,padx=30)
    



    return bucket, run_offer
